...
Note that we should not condition on $$\theta("p(y^{(i)}|x^{(i)};\theta)")$$, since $$\theta$$ is not a random variable. 
We can also write the distribution of $$y^{(i)}$$ as $$y^{(i)}|x^{(i)};\theta \sim \mathcal{N}(\theta^\mathrm{T}x^{(i)}, \sigma^2)$$.
